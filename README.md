...
- **攻坚排表**：
  ...
  - 报名制：未锁定攻坚可报名/取消报名；团长（发起人）默认参加；管理员可帮成员报名（报名列表加号，锁定后不可）；未报名用户占位时先引导报名，管理员排表仅可选已报名用户的角色；详情页展示已报名名单并支持取消（自动撤下占位）；报名人数含团长（列表/详情口径一致）
  - 占位交互：占位弹窗与成员头像只读弹窗以角标标记「已占位 · 第N波 · 某队」；详情页报名名单点击头像可只读查看该成员角色与占位情况
  - 攻坚信息：名称/时间可修改，WebSocket 实时同步
  - 非锁定状态下任何成员可加波、删除仅含自己角色的波；管理员可锁定/解锁、编辑任意格子
- **我的角色**：编辑框打开时点击角色卡片即关闭（等同取消），移动端无需下滑到取消按钮
- **实时同步**：WebSocket 增量广播，断线自动重连
- **机器人 API**：固定 Token 供群机器人调用——按账号/昵称查询与录入角色、按 QQ 号自助注册、查询攻坚列表与完整波次详情（配套 AstrBot skill：dnfer-characters / dnfer-raids）
