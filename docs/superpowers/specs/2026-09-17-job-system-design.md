...
- 职业数据由后端提供：后端加载仓库根 `职业信息.json`，新增 `GET /api/jobs` 返回过滤后的职业树，且每个子职业**携带推导好的 `class_type`**；前端选职业时据此判定输入区，**不在前端镜像辅助职业集合**（单一来源，消除漂移）。
- 图标由后端托管：`images/adventure` 挂载为 `/images` 静态路由；大类图标 = `/images/sub/{name}.png`，子职业图标 = `/images/jobs/{name}.png`。
- 缺图回退：已知 `breaker`、`infighter_female`（子职业）、`imperialknight`（大类）无对应 PNG；前端图标加载失败时回退到占位图 `/images/jobs/empty.png`，后续补齐图片即自动恢复。
...
返回 `job_tree()`（大类 `{id, name, title, children}`，子项 `{id, name, title, class_type}`）。前端选职业用；子项 `class_type` 由后端推导，前端据此刻画输入区。若某大类过滤空子职业后无子项，该大类不可选（防御；当前数据每类均含真实子职业）。

路由归属：新文件 `backend/app/routers/jobs.py`（prefix `/api/jobs`，依赖 `get_current_user`），**不**放进 `public.py`（其路由级依赖是机器人 Token，认证语义不符）。
...
- 新增 `JobChild`（含 `class_type`）、`JobCategory` 响应模型。
- `class_type` 推导位置固定：`members.py` 的 create/update 处理器中 `class_type = jobs.class_type_for(body.job_name)` 后写入 `Character`。
...
- `types.ts`：`Character` 增 `job_name`/`job_title`/`parent_name`；`Slot` 增 `job_name`/`job_title`；新增 `JobChild`（含 `class_type`）、`JobCategory`。
- 新增 `src/lib/job.ts`：
...
- 列表每行：显示子职业图标（`jobIcon`，加载失败回退占位图）+ 职业名（`job_title`）+ 原 class_type / 名望 / 数值展示（行为不变）。
- 添加/编辑表单：职业选择器替换原「输出/辅助」下拉：
  1. 大类图标网格（`categoryIcon` + title，加载失败回退），点击选中大类；
  2. 展示该大类子职业图标网格（`jobIcon` + title），点击选中具体职业；
  3. 按所选子职业 `class_type`（取自 `GET /api/jobs` 下发数据）显示对应输入区（模拟伤害+秒伤 / 增益量），并提示职业类型。
- 编辑回填：根据角色 `job_name` 反推父类并预选。
- 保存提交 `job_name`（不再提交 `class_type`）。

### 5.3 `CharacterPickerModal`（攻坚选角色）

- 每行显示子职业图标（加载失败回退占位图）+ 职业名（便于辨认），数值展示不变。

### 5.4 `SlotCell`（攻坚排表）

- 占位格显示子职业小图标（加载失败回退占位图）+ 职业名（在角色名旁），数值/职责展示不变。
...
2. 仅在**本次新增该列时**（存量库首次迁移）执行清空，**必须先清 slots 再删 characters**——SQLite 已启用 `PRAGMA foreign_keys=ON`，`Slot.character_id` 外键无 `ON DELETE` 动作，先删角色会触发外键约束错误：
   - `UPDATE slots SET character_id = NULL, duty = NULL, version = version + 1, updated_by = NULL, updated_at = NULL`
...
- **后端 pytest**：
  ...
  - 非法 `job_name` → 400。
  - `GET /api/jobs` 返回职业树、无 `empty` 条目，且子项带推导的 `class_type`（5 个辅助职业为「辅助」）。
  - 回归：辅助职业占位默认职责为主奶、`duty_valid_for_class` 仍生效；`SlotOut` 含 `job_name`/`job_title`。
  - 迁移：`migrate_jobs` 加列 + 清空角色/占位。
- **前端 Vitest**：
  - `lib/job.spec.ts`：图标 URL 拼接（`jobIcon`/`categoryIcon`）、缺图回退逻辑。
  - `MyCharactersView` 表单测试：选职业后按子职业 `class_type`（取自 `GET /api/jobs`）显示正确输入区、提交 `job_name`（不镜像辅助职业集合）。
...
- `backend/app/routers/members.py` — create/update 校验 `job_name`、推导 `class_type`、显式构造 `CharacterOut`
- `backend/app/routers/jobs.py` — 新路由 `GET /api/jobs`（`get_current_user`）
- `backend/app/routers/raids.py` — `_slot_out` 补充 `job_name`/`job_title`
- `backend/app/main.py` — 挂载 `/images`、注册 jobs 路由
